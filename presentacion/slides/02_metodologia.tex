...
\begin{frame}{Enfoque metodológico: desarrollo por prototipos}
    \begin{columns}[T]
        \begin{column}{0.45\textwidth}
            ...
            \begin{itemize}
                \item \textbf{Naturaleza exploratoria:} tecnologías emergentes sin antecedentes locales
                \item \textbf{Requisitos evolutivos:} marco normativo en constante cambio
                \item \textbf{Verificación temprana:} validar hipótesis antes de escalar
            \end{itemize}
        ...
        \begin{column}{0.55\textwidth}
            \centering
            \textbf{Ciclo iterativo de prototipado}
            ...
            \begin{tikzpicture}[
                node distance=0.5cm,
                fase/.style={rectangle, rounded corners, draw=udblue, fill=udblue!10, text width=3.5cm, minimum height=0.65cm, align=center, font=\fontsize{7}{8}\selectfont\bfseries},
                flecha/.style={->, thick, udlightblue},
                retro/.style={->, thick, udorange, dashed}
            ]
                \node[fase] (req) {Análisis de requisitos\\[-1pt]\fontsize{5}{6}\selectfont Marco legal + auditorías};
                \node[fase, below=of req] (dis) {Diseño arquitectónico\\[-1pt]\fontsize{5}{6}\selectfont Niveles de confianza};
                \node[fase, below=of dis] (imp) {Implementación\\[-1pt]\fontsize{5}{6}\selectfont Backend + Frontend + SC};
                \node[fase, below=of imp] (pru) {Pruebas y verificación\\[-1pt]\fontsize{5}{6}\selectfont 80 casos automatizados};
                \node[fase, below=of pru] (eval) {Evaluación\\[-1pt]\fontsize{5}{6}\selectfont Validación de requisitos};

                \draw[flecha] (req) -- (dis);
                \draw[flecha] (dis) -- (imp);
                \draw[flecha] (imp) -- (pru);
                \draw[flecha] (pru) -- (eval);

                % Flechas de retroalimentación
                \draw[retro] (eval.east) to[out=0,in=0] node[right, font=\fontsize{4.5}{5.5}\selectfont, text=udorange] {Retro.} (dis.east);
                \draw[retro] (pru.west) to[out=180,in=180] node[left, font=\fontsize{5}{6}\selectfont, text=udorange] {Refinamiento} (req.west);
            \end{tikzpicture}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Arquitectura híbrida: decisión de diseño}
    \textbf{Problema:} una sola plataforma blockchain no satisface simultáneamente todos los requisitos.
...
\begin{frame}{Arquitectura del sistema}
    ...
    \footnotesize
    \textbf{Capas:} \textbf{1.} Frontend React ~|~ \textbf{2.} API REST Node.js/Express ~|~ \textbf{3.} Hyperledger Fabric (privado) ~|~ \textbf{4.} Ethereum (público) ~|~ \textbf{5.} IPFS (almacenamiento distribuido)
\end{frame}
...
\begin{frame}{Alcance y delimitaciones del estudio}
    ...
    \begin{itemize}
        \item \textbf{Datos sintéticos:} la verificación se realizó con datos generados mediante scripts de prueba, dado que no se dispuso de acceso a datos reales del FÉNIX, RUNT ni SIMIT.
