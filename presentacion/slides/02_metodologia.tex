...
\begin{frame}{Enfoque metodológico: desarrollo por prototipos}
    \begin{columns}[T]
        ...
        \begin{column}{0.55\textwidth}
            ...
            \begin{tikzpicture}[
                node distance=0.6cm,
                fase/.style={rectangle, rounded corners, draw=udblue, fill=udblue!10, text width=4cm, minimum height=0.7cm, align=center, font=\footnotesize\bfseries},
                flecha/.style={->, thick, udlightblue},
                retro/.style={->, thick, udorange, dashed}
            ]
                \node[fase] (req) {1. Análisis de requisitos\\[-1pt]\fontsize{5.5}{6.5}\selectfont Marco legal + auditorías};
                \node[fase, below=of req] (dis) {2. Diseño arquitectónico\\[-1pt]\fontsize{5.5}{6.5}\selectfont Niveles de confianza};
                \node[fase, below=of dis] (imp) {3. Implementación iterativa\\[-1pt]\fontsize{5.5}{6.5}\selectfont Backend + Frontend + Smart Contracts};
                \node[fase, below=of imp] (pru) {4. Pruebas y verificación\\[-1pt]\fontsize{5.5}{6.5}\selectfont 80 casos automatizados};
                ...
                \draw[retro] (pru.east) to[out=0,in=0] node[right, font=\fontsize{5}{6}\selectfont, text=udorange] {Retroalimentación} (dis.east);
                \draw[retro] (imp.west) to[out=180,in=180] node[left, font=\fontsize{5}{6}\selectfont, text=udorange] {Refinamiento} (req.west);
            \end{tikzpicture}
        \end{column}
    \end{columns}
\end{frame}
