...
El prototipo implementa cinco de los ocho estados teóricos, priorizando aquellos con impacto directo en la integridad y trazabilidad del proceso sancionatorio. La matriz siguiente resume el criterio de selección:
...
\hline
\end{tabular}
\caption{Cobertura de estados: sólo se implementan los que afectan la integridad y trazabilidad.}
\label{tab:cobertura_estados_justificacion}
\end{table}

\subsubsection{Estados excluidos}
\paragraph{NOTIFICADA} Requiere verificación física mediante oráculos externos, fuera del alcance experimental.\newline
\paragraph{CERRADA} Estado administrativo sin impacto en la integridad; su omisión no afecta la validez técnica.

\subsubsection{Validación técnica}
Las pruebas demuestran que en los cinco estados implementados:
\begin{itemize}
...
En síntesis, la selección de estados responde a criterios técnicos de impacto y viabilidad experimental. La omisión de NOTIFICADA y CERRADA es metodológica y no compromete la validez científica del prototipo.