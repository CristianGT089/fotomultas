...
\subsubsection{Significado técnico de "Exitoso" en blockchain}
En sistemas tradicionales, "Exitoso" indica ausencia de errores. En blockchain, implica validación de garantías criptográficas: la inmutabilidad y trazabilidad están protegidas matemáticamente, no solo por lógica de software. Por ejemplo, la prueba IM-001 demuestra que cualquier intento de modificar registros es rechazado por el consenso, asegurando integridad.

\subsubsection{Verification vs. Validation}
\textbf{Verification} evalúa si el sistema cumple la especificación; \textbf{Validation} determina si resuelve el problema real. El éxito en pruebas corresponde a Verification. Validation en producción requiere superar limitaciones técnicas y operativas.
...
\paragraph{Aspectos validados}
\begin{itemize}
    \item \textbf{Viabilidad técnica:} La arquitectura híbrida cumple con los mecanismos de consenso y trazabilidad.
    \item \textbf{Gobernanza criptográfica:} Las reglas de negocio son forzadas matemáticamente.
    \item \textbf{Trazabilidad auditable:} Todas las transiciones quedan registradas y verificables.
    \item \textbf{Abstracción técnica:} Blockchain se integra mediante APIs REST convencionales.
\end{itemize}
\paragraph{Aspectos no validados}
\begin{itemize}
    \item \textbf{Rendimiento a gran escala:} No se ha probado con volúmenes masivos.
    \item \textbf{Robustez con datos reales:} Falta validación con datos heterogéneos.
    \item \textbf{Seguridad avanzada:} No se han realizado auditorías externas.
    \item \textbf{Operación y mantenimiento:} Requiere equipos y procedimientos formales.
    \item \textbf{Integración institucional:} Las APIs simuladas no reflejan la complejidad real.
\end{itemize}

\subsubsection{Alineación con las variables del problema de investigación}
Los cinco estados validados (PENDING, PAID, APPEALED, RESOLVED\_APPEAL, CANCELLED) corresponden a las transiciones donde la manipulación de datos tiene impacto directo en las cuatro variables identificadas en la Formulación del problema: la tasa de impugnación del 34.1\% se mitiga al garantizar inmutabilidad desde PENDING; la carga de 155,854 PQRSD semestrales se aborda al proporcionar trazabilidad completa en APPEALED$\rightarrow$RESOLVED\_APPEAL; la vulnerabilidad ciudadana se reduce mediante auditoría de CANCELLED; y el potencial detrimento patrimonial disminuye al eliminar posibilidades de fraude en todas las transiciones críticas.

La metodología de desarrollo por prototipos permite que esta base inicial evolucione según las necesidades operativas. El diseño extensible del \textit{smart contract} (mediante el tipo enumerado \texttt{FineState}) facilita agregar estados como NOTIFICADA o CERRADA en iteraciones futuras sin rediseñar la arquitectura.
