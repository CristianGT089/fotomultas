...
Esta sección presenta un análisis crítico de los resultados obtenidos, interpretando los hallazgos en el contexto de los objetivos planteados y comparándolos con el estado del arte revisado. Se discuten las implicaciones de la arquitectura híbrida implementada, sus ventajas sobre sistemas tradicionales y las lecciones aprendadas durante el desarrollo del prototipo.

El trabajo logró desarrollar exitosamente un prototipo para apoyar el registro y trazabilidad de estados en el proceso de fotocomparendos en Bogotá, aplicando tecnologías de redes distribuidas para fortalecer la integridad, la autenticidad de la información y reducir los riesgos asociados a su confidencialidad. Este objetivo fue alcanzado mediante la implementación de una arquitectura híbrida blockchain que combina Hyperledger Fabric para gestión privada de datos sensibles con Ethereum para transparencia pública.

Los resultados de las pruebas de inmutabilidad (Resultados de las pruebas de inmutabilidad y verificabilidad del prototipo) demuestran que el sistema garantiza la integridad de los registros mediante hash criptográficos inmutables en blockchain y CIDs verificables en IPFS. La autenticidad se asegura mediante firmas digitales y control de acceso basado en certificados en Hyperledger Fabric, mientras que la confidencialidad se preserva manteniendo datos sensibles exclusivamente en la capa privada.

En cuanto a la implementación de tecnología blockchain para garantizar la inmutabilidad de los registros de fotocomparendos, se desarrollaron dos blockchains complementarias: Hyperledger Fabric para registros privados completos con consenso PBFT que garantiza finalidad inmediata de transacciones, y Ethereum para metadatos públicos con consenso Proof-of-Stake que asegura inmutabilidad verificable públicamente.

Las pruebas de inmutabilidad (la Tabla~\ref{tab:resumen_inmutabilidad}) demostraron que cualquier intento de modificación directa del ledger es rechazado automáticamente por el mecanismo de consenso. La arquitectura de bloques encadenados mediante hashes criptográficos hace que la alteración de registros pasados sea computacionalmente prohibitiva.

Para el almacenamiento verificable de evidencias fotográficas, se implementó una arquitectura dual de IPFS que incluye un nodo privado para almacenar evidencias fotográficas completas con resolución original y un nodo público que publica hashes de evidencias para verificación ciudadana. Los resultados de las pruebas de verificación de integridad (Resultados de las pruebas de inmutabilidad y verificabilidad del prototipo) confirmaron que el sistema de direccionamiento por contenido (CID) de IPFS detecta automáticamente cualquier alteración, incluso mínima, en las evidencias.

El 100\% de las verificaciones de integridad fueron exitosas, validando que IPFS proporciona un mecanismo robusto para garantizar que las evidencias no han sido manipuladas.

El desarrollo de una API REST que integre blockchain con aplicaciones web tradicionales se materializó en un backend completo con Express.js que expone 6 endpoints principales documentados con Swagger. La API actúa como capa de abstracción entre las blockchains y el frontend, permitiendo registro de multas con upload automático a IPFS y transacción en ambas blockchains, consultas diferenciadas (privadas desde Fabric, públicas desde Ethereum), verificación de integridad cruzada entre blockchains, y sincronización transparente de datos públicos.

Los tests de integración (Resultados de las pruebas de inmutabilidad y verificabilidad del prototipo) demostraron que la API maneja correctamente tanto operaciones síncronas (consultas) como asíncronas (sincronización blockchain), cumpliendo con tiempos de respuesta aceptables (<3 segundos para registro completo).

Finalmente, se desarrolló una interfaz web intuitiva para agentes y ciudadanos implementada en React con tres módulos diferenciados: Panel de Agente para registro y gestión de multas, Panel Ciudadano para consulta pública y verificación de integridad, y Dashboard Administrativo para visualización de estadísticas con Recharts. La implementación con Tailwind CSS proporciona una interfaz moderna y responsive.

El uso de Zustand para gestión de estado y React Router para navegación demuestra que las tecnologías blockchain pueden integrarse con interfaces de usuario contemporáneas sin comprometer la experiencia del usuario.

\subsection{Arquitectura híbrida}
...
El uso de Hyperledger Fabric para operaciones frecuentes elimina los costos de gas asociados a blockchains públicas. Según las métricas de rendimiento (la Tabla~\ref{tab:rendimiento}), las transacciones en Fabric tienen latencia 10x menor que en Ethereum, permitiendo mayor volumen de operaciones a menor costo.
...
La Tabla~\ref{tab:comparacion_fenix} presenta una comparación detallada entre el sistema FÉNIX actual y la arquitectura híbrida propuesta.

\input{tables/comparacion_fenix.tex}

\noindent Es importante resaltar que, para el sistema FÉNIX, no se cuenta con acceso a métricas técnicas detalladas de rendimiento ni a su arquitectura interna; por tanto, las comparaciones cuantitativas se basan en la tasa de PQRSD, hallazgos de auditoría y percepciones documentadas en informes oficiales. En contraste, los valores reportados para el prototipo provienen de medidas directas en entorno de laboratorio y deben interpretarse como un escenario de referencia para estimar el potencial de mejora, más que como un reemplazo inmediato del sistema actual.

\subsubsection{Comparación con estado del arte}

Al contrastar el presente trabajo con los sistemas revisados en la sección Metodologíca, se identifican las siguientes diferencias:
...
\subsection{Implicaciones y contribuciones}
...
La capacidad de verificación pública sin intermediarios aborda directamente el problema de la desconfianza ciudadana. Una tasa de impugnación del 34.1\% (identificada en la sección de Introducción) sugiere que la falta de transparencia actual genera fricciones masivas. El acceso a verificación automática de integridad podría reducir significativamente las PQRSD no justificadas.

\paragraph{Reducción de carga administrativa}
Las 155,854 PQRSD procesadas semestralmente (datos de la sección Introducción) representan una carga operativa significativa. La verificación automática de integridad permitiría a los ciudadanos validar por sí mismos la autenticidad de multas, reduciendo potencialmente en más del 50\% las solicitudes relacionadas con dudas sobre la validez de los registros.

\subsubsection{Impacto económico}

\paragraph{Análisis costo-beneficio}
Según el análisis de costos de la sección Implementación del prototipo, la implementación del prototipo requiere una inversión inicial moderada, pero genera ahorros en:
\begin{itemize}
...
\subsection{Del éxito técnico a la validación operativa: Interpretación crítica de resultados}

\subsubsection{¿Qué significa ``Exitoso'' en el contexto de blockchain?}

Un resultado de prueba ``Exitoso'' (100\% de casos pasando) en un sistema tradicional significa que el software no falló bajo las condiciones de prueba. En un sistema blockchain, significa algo más profundo: \textbf{una garantía criptográfica ha sido validada}.

Consideremos la prueba de inmutabilidad IM-001 (\textit{``Intentar modificar un hash registrado en blockchain''}):

\begin{itemize}
    \item \textbf{En una BD tradicional:} Un resultado ``Exitoso'' significa que la consulta ejecutó sin error, pero no garantiza que un atacante con acceso administrativo no pueda cambiar el registro (caso de fraude en sistemas FÉNIX).
    
    \item \textbf{En nuestro sistema blockchain:} Un resultado ``Exitoso'' significa que \textit{matemáticamente}, bajo las leyes de criptografía de curvas elípticas y funciones hash criptográficas, es computacionalmente prohibitivo cambiar el registro sin invalidar toda la cadena. Alguien intentó cambiar un bit del hash registrado, y el sistema lo rechazó de manera \textbf{determinista}, no por coincidencia de programación.
\end{itemize}

\textbf{Conclusión:} El éxito en pruebas IM-001 valida que \textbf{la gobernanza está forzada matemáticamente}, no solo por software. Esto es el propósito central del proyecto.

\subsubsection{Dos conceptos críticos: Verification vs. Validation}

En ingeniería de software, existen dos conceptos distintos que frecuentemente se confunden:

\begin{itemize}
    \item \textbf{Verification:} ``¿Lo estamos construyendo correctamente?'' ¿El código implementa la especificación? ¿Las pruebas pasan?
    
    \item \textbf{Validation:} ``¿Estamos construyendo lo correcto?'' ¿El sistema resuelve el problema del usuario? ¿Funciona en el contexto real?
\end{itemize}

El 100\% de éxito en nuestras pruebas valida \textbf{Verification}. Demuestra que:

\begin{itemize}
    \item El código implementa correctamente los smart contracts especificados
    \item La arquitectura híbrida funciona como se diseñó
    \item Todos los mecanismos de consenso y criptografía operan correctamente
\end{itemize}

Sin embargo, la \textbf{Validation} completa para producción requiere superar brechas adicionales identificadas en la sección 11.4 (Limitaciones). Esto es lo que significa que ``Exitoso no basta''.

\subsubsection{Matriz de Brechas: Entorno de Prueba vs. Producción}

El siguiente cuadro presenta analíticamente las diferencias entre el contexto donde validamos (laboratorio) y donde se implementaría (Bogotá):
...
\hline
\textbf{Volumen de multas} & 50-100 & 457,000 semestrales & 1000x \\
\hline
\textbf{Datos} & Sintéticos (formato uniforme) & Reales (inconsistencias, excepciones) & Variabilidad desconocida \\
\hline
\textbf{Blockchain Ethereum} & Testnet Hardhat (local) & Mainnet público & Latencia 10-30s vs. 2.7s \\
\hline
\textbf{IPFS} & Nodo local pinning 100\% & Cluster distribuido con retención & Complejidad operacional \\
\hline
\textbf{APIs externas (RUNT/SIMIT)} & Mock (respuesta instantánea) & Real (latencias 200-500ms) & Cascada de timeouts \\
\hline
\textbf{Auditoría de seguridad} & Testing interno (26 casos) & Auditoría formal 3eras & Vulnerabilidades no detectadas \\
\hline
\textbf{Gestión de claves} & Archivos .env & Hardware Security Modules & Riesgo de compromiso \\
\hline
\textbf{SLA de disponibilidad} & No aplicable & 99.9\% (27 min downtime/mes) & Resiliencia no probada \\
\hline
\end{tabular}
\caption{Brechas Verification-Validation: Laboratorio vs. Producción. El éxito del prototipo se valida en la columna central; el riesgo en producción está en la columna derecha.}
\label{tab:verification_validation_gaps}
\end{table}

\subsubsection{Interpretación: Lo que el éxito técnico demuestra y lo que no}

\paragraph{Lo que SÍ valida el prototipo exitoso}

\begin{itemize}
    \item \textbf{Viabilidad de diseño:} La arquitectura híbrida es técnicamente sonida bajo el consenso PBFT de Fabric y PoS de Ethereum
    
    \item \textbf{Gobernanza criptográfica:} Es posible usar blockchain para forzar reglas de negocio (p.ej., no borrar multas) de manera que no puedan ser violadas por humanos sin detección
    
    \item \textbf{Trazabilidad auditable:} Todos los cambios quedan registrados y son verificables públicamente sin intermediarios
    
    \item \textbf{Abstracción técnica:} Las complejidades de blockchain pueden abstraerse detrás de APIs REST convencionales
\end{itemize}

\paragraph{Lo que NO valida el prototipo (aún no})}

\begin{itemize}
    \item \textbf{Rendimiento a escala real:} El sistema puede procesar 100 multas en laboratorio, pero ¿procesa 1 millón sin degradación?
    
    \item \textbf{Robustez con datos reales:} Datos sintéticos están ``limpios''; datos reales de ciudadanos tienen inconsistencias, placas extranjeras, excepciones
    
    \item \textbf{Resistencia ante ataques avanzados:} Las pruebas validaron ataques básicos (XSS, SQL injection); no valida sofisticados (exploits de firmware, ataques de tiempo)
    
    \item \textbf{Mantenibilidad operacional:} El prototipo fue construido por desarrolladores investigadores; la operación real requiere equipos, procedimientos, SLAs
    
    \item \textbf{Integración con ecosistema:} El mock de RUNT/SIMIT no captura su complejidad real, latencias, formatos excepcionales
\end{itemize}

\subsubsection{Conclusión: Éxito como condición necesaria pero no suficiente}

El estado ``Exitoso'' de las pruebas es una \textbf{condición necesaria} pero no \textbf{suficiente} para validación de producción. Analogía en medicina:

\begin{itemize}
    \item Un medicamento pasa ensayos clínicos de Fase III (100\% de pacientes sin reacciones adversas graves) — esto es ``Exitoso''
    \item Pero antes de prescribirse masivamente, pasa Fase IV (monitoreo farmacovigilancia post-lanzamiento) para detectar efectos raros
\end{itemize}

De manera similar, nuestro prototipo debe:

\begin{enumerate}
    \item Validar exitosamente todos sus componentes (completado ✓)
    \item Pasar auditorías de seguridad formal (pendiente)
    \item Ejecutarse bajo carga real de 457,000 multas/semestre (pendiente)
    \item Integrarse con sistemas gubernamentales reales (pendiente)
    \item Operarse bajo SLAs de disponibilidad (pendiente)
\end{enumerate}

El presente trabajo completa el paso 1. El éxito técnico reportado es genuino y significativo — demuestra que blockchain \textit{puede} ser la solución. Pero la implementación en Bogotá requiere completar los pasos 2-5 en una fase operativa posterior.
