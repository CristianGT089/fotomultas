...
\begin{enumerate}
  \item \textbf{Registro inmutable de la infracción}
        Captura de metadatos (placa, fecha, hora, ubicación y tipo de infracción) y publicación del identificador de la evidencia en la \emph{blockchain} (\textit{Ethereum} local con \textit{Hardhat} para desarrollo, con arquitectura preparada para \textit{Hyperledger Fabric} en producción).
  \item \textbf{Almacenamiento descentralizado de evidencias}  
        Carga de la imagen o video de la fotomulta en \textit{IPFS} y obtención de su \emph{hash}.
  \item \textbf{Verificación pública}  
        Servicio de consulta que permite contrastar el \textit{hash} guardado en la cadena con el archivo almacenado en \textit{IPFS}.
  \item \textbf{Gestión del ciclo de vida de la multa}
        El prototipo implementa la gestión de transiciones entre estados del proceso de fotocomparendos. El modelo conceptual completo, descrito en la Formulación del problema (sección 1.1), contempla ocho estados: GENERADA $\rightarrow$ NOTIFICADA $\rightarrow$ PENDIENTE\_RESPUESTA $\rightarrow$ EN\_APELACION $\rightarrow$ RESUELTA\_APELACION $\rightarrow$ PAGADA/CANCELADA $\rightarrow$ CERRADA. Como prueba de concepto, el \textit{smart contract} implementa cinco de estos estados (ver Tabla~\ref{tab:mapeo_estados} en sección 8 para mapeo detallado). Cada transición queda registrada mediante eventos de \textit{smart contract} con \textit{timestamp} inmutable.
...
\begin{itemize}
  \item \textbf{Contrato inteligente en Solidity} con 80 pruebas automatizadas (97.5\% de éxito).
  \item Script de despliegue de red \textit{Ethereum} local (\textit{Hardhat}) e instalación de \textit{IPFS} local.
...
\begin{enumerate}
  ...
  \item Coincidencia 100 \% entre el \textit{hash} almacenado en la cadena y la evidencia recuperada desde \textit{IPFS}.
