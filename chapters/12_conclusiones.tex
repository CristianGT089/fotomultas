...
Si bien el prototipo desarrollado valida de manera rigurosa la inmutabilidad en los estados críticos del proceso (PENDING, PAID, APPEALED, RESOLVED_APPEAL, CANCELLED), la transición al entorno operativo exige abordar una brecha conceptual relevante: la certificación del estado NOTIFICADA, correspondiente a la notificación física y fehaciente al ciudadano.
...
\begin{enumerate}
    ...
    \item \textbf{Cierre de la cadena de custodia:} Con la certificación del estado NOTIFICADA mediante oráculo, y la gestión inmutable de los estados PENDING, PAID, APPEALED, RESOLVED_APPEAL y CANCELLED en blockchain, se logra una trazabilidad integral y auditable de todo el ciclo de vida del fotocomparendo (GENERADA → NOTIFICADA → [proceso] → PAGADA/CANCELADA → CERRADA).
\end{enumerate}
